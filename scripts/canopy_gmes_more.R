...
# ternary correction factor according to Cernusak et al. 2012
# assuming boundary layer resistance is negligible
calcTern <- function(a, gsc, E){
  t <- (1+a)*E/(2*gsc)
  return(t)
}

# DELTAi is photosynthetic discrimination against 13C due to biochemical fractionation
# and diffusion fractionation and excluding boundary layer, 
# internal transfer, respiration and photorespirataion
calcDELTAi <- function(a, b, Ci.Ca, t){
  DELTAi <- (1/(1-t))*a+(1/(1-t))*((1+t)*b-a)*(Ci.Ca)
  return(DELTAi)
}
...
# DELTAe is discrimination due to respiration
calcDELTAe <- function(t, eResp, Rd, Photo, CO2sample, Ci, gammaStar){
  DELTAe <- ((1+t)/(1-t))*((eResp*Rd/((Photo+Rd)*CO2sample))*(Ci-gammaStar))
  return(DELTAe)
}
# DELTAf is photoresiration fractionation
calcDELTAf <- function(t, f, gammaStar, CO2cuv){
  DELTAf <- ((1+t)/(1-t))*(f*(gammaStar/CO2cuv))
  return(DELTAf)
}
