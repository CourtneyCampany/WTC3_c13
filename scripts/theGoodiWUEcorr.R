...
del13CcampAvg <- doBy::summaryBy(iWUEge_corr + iWUE + Corrdel13C_Avg + CO2sampleWTC + Cc + diff_Ci.Cc ~ month + chamber,
                                 FUN=mean.na, data=subset(allPaired, PAR > 3 & condAlert=='no'
                                                          & A_area > 0 & E_area > 0 & iWUE < 500))
del13CcampAvgMD <- doBy::summaryBy(iWUEge_corr + iWUE + Corrdel13C_Avg + CO2sampleWTC + diff_Ci.Cc ~ month + chamber,
                                   FUN=mean.na, data=subset(allPaired, midday=='yes' & condAlert=='no' &
                                                              A_area > 0 & E_area > 0& iWUE < 500))
names(del13CcampAvgMD)[3:ncol(del13CcampAvgMD)] <- paste0(names(del13CcampAvgMD)[3:ncol(del13CcampAvgMD)], 'MD')
phl <- merge(merge(merge(del13CcampAvgMD, del13CcampAvgDay, by=c('month','chamber'), all=T),
                   del13CcampAvg, by=c('month','chamber'), all=T), phl, by=c('month','chamber'), all=T)
...
iWUEsumm <- doBy::summaryBy(iWUE.mean.naMD + iWUE.mean.na + iWUEph_corrMD + iWUEph_uncorrMD +
                              iWUEleafAvg_corrMD ~ month + T_treatment, data=phl, FUN=c(mean.na, s.err.na))
