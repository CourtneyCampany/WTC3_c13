...
leafGmesAvg1 <- doBy::summaryBy(leaf_gmes ~ chamber + month, FUN=mean.na, data=subset(leafGmes, leaflight!='shade-high'))
names(leafGmesAvg1)[3] <- 'leaf_gmesAvg1'
leafGmesAvg2 <- doBy::summaryBy(leaf_gmes ~ chamber + month, FUN=mean.na, data=leafGmes)
